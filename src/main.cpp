...
void read_config(Filenames &fns,Parameter &para, Weight &weight, const string &config_file)
{
	...
	while(getline(fin,line))
	{
		...
		else if (line == "[BEAM_SIZE]")
		{
			getline(fin,line);
			para.BEAM_SIZE = stoi(line);
		}
		else if (line == "[EXTRA_SIZE]")
		{
			getline(fin,line);
			para.EXTRA_SIZE = stoi(line);
		}
		else if (line == "[NBEST_NUM]")
		{
			getline(fin,line);
			para.NBEST_NUM = stoi(line);
		}
		else if (line == "[REORDER_WINDOW]")
		{
			getline(fin,line);
			para.REORDER_WINDOW = stoi(line);
		}
		else if (line == "[SIZE_LIMIT]")
		{
			getline(fin,line);
			para.SIZE_LIMIT = stoi(line);
		}
		else if (line == "[PRINT_NBEST]")
		{
			getline(fin,line);
...
int main()
{
	...
	Parameter para;
	Weight weight;
	read_config(fns,para,weight,"config");

	Vocab *src_vocab = new Vocab(fns.src_vocab_file);
	Vocab *tgt_vocab = new Vocab(fns.tgt_vocab_file);
	RuleTable *ruletable = new RuleTable(para.SIZE_LIMIT,weight,fns.rule_table_file);
